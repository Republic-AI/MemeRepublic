...
import com.infinity.ai.quartz.task.LogoutTask;
import com.infinity.ai.quartz.task.RefreshTask;
import com.infinity.ai.quartz.task.SubmitCronTask;
...
import static com.infinity.common.msg.ProtocolCommon.MSG_CODE_TIMER_SUBMIT;
...
public class NewTaskFactory implements ITaskFactory {
    ...
    static {
        ...
        NewTaskFactory.register(MSG_CODE_TIMER_SUBMIT, SubmitCronTask::new);
    ...
    public ITask createTask(int command, HeaderOuterClass.Header header, byte[] buffer, IChannel channel, Object attachement) {
        ...
        if (creator != null) {
            ...
            log.error("failed to creat task. cmd={},header={}",
                    command, header.toString());
        }
        return null;
    }
}
